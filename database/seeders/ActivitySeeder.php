...
class ActivitySeeder extends Seeder
{
    public function run(): void
    {
        ...
        $questions = [
            1 => [ // Module 1: Gun Parts (20 questions)
            ...
            2 => [ // Module 2: Marksmanship & Firing Techniques (20 questions)
            ...
            3 => [ // Module 3: Maintenance (10 questions)
        ...
        foreach ($questions as $moduleNum => $qList) {
            $moduleKey = $moduleMap[$moduleNum];
            $module = Module::where('module_key', $moduleKey)->first();
            if (!$module) {
                $this->command->warn("Module $moduleKey not found, skipping");
                continue;
            }

            $pageIds = LessonPage::whereIn('lesson_id', Lesson::where('module_id', $module->id)->pluck('id'))
                ->orderBy('lesson_id')
                ->orderBy('page_index')
                ->pluck('id');

            if ($pageIds->isEmpty()) {
                $this->command->warn("No lesson pages found for module $moduleKey, skipping");
                continue;
            }

            foreach ($qList as $i => $q) {
                Activity::create([
                    'lesson_detail_id' => $pageIds[$i % $pageIds->count()],
                    'question_number' => $i + 1,
                    'question_text' => $q['q'],
                    'options' => $q['o'],
                    'correct_answer' => $q['c'],
